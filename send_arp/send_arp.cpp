...
{
	//at first argument num is  "filename dev senderIp targetIp"
	if (argc != 4) {
	...
	pcap_t * pd = pcap_open_live(pack_info.dev_name, BUFSIZ, 1, 1000, errbuf);//1sec
...
void get_my_info(char * dev_name, uint32_t *struc_p, u_char *struc_mac)
{
	...
	memcpy(struc_p, ifr.ifr_addr.sa_data + 2, sizeof(uint32_t));// Get IP Adress
	memcpy(struc_mac, ifr.ifr_hwaddr.sa_data, sizeof(u_char) * 6);//get mac addr
